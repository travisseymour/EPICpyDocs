...
Version 3.9.7 (currently developed with 3.9.7, but should work with later versions of 3.9.x)

Note: EPICpy may work fine on Python versions beyond 3.9.x, but this has not been tested. The critical question is whether the modules and packages EPICpy depends on will work for Python 3.10 and beyond. This notice will be updated as newer Python versions are tested and verified as working.
...
Some or most of these may already be all installed on your system:
...
1. Installing Python 3.9.7 using pyenv
...
- Install Python 3.9.7
    ...
    ```
- On at least one Ubuntu machine pyenv may complained that required programs were not already installed on the system. If so, install the ones it asks for. In this one case, the following was sufficient...but it may not be required for everyone!:
   ```bash
...
don't forget to delete the cppyy_build folder when done, e.g., ``rm -rf cppyy_build``.
...
Navigate back out to the root `EPICpy` folder. From here issue this command (it is assumed that the virtual environment is still activated):
    ...
    python onetime_resource_setup.py 
...
`Only tested on MacOS version 10.15 and version 11 (Intel CPU)`

**Install Prerequisites**

The only prerequisite is to install XCode. See the MacOS section of [this page](installing.md) for more information about how to do download and install XCode (make sure you run it at least once post install).
